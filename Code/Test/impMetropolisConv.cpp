                          ...
                          MatrixXd& S, int index, Butcher butcher)
{
    ...
    impProbMethod solver(odeModel, h, param, sigma, butcher.getA(), butcher.getB(), 4);
    ...
    for (int i = 1; i < nStepsMC; i++) {
        ...
        for (MCindex = 0; MCindex < internalMC; MCindex++) {
            impProbMethod solverNew(odeModel, h, paramStd, sigma, butcher.getA(), butcher.getB(), 4);
        ...
        for (MCindex = 0; MCindex < internalMC; MCindex++) {
            impProbMethod solverOld(odeModel, h, paramStd, sigma, butcher.getA(), butcher.getB(), 4);
            std::vector<VectorXd> solutionAtDataTimesPar(nData, VectorXd(odeModel.size));
            double t = 0;
            int count = 0;
            while (t < finalTime) {
