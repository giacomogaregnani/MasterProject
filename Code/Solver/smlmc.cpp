...
{		
	...
	if (!fromTimestep) {
		nLevels = 1 + ceil(1.0 / (double) (order) * abs(log2(accuracy)));
	} else {
		nLevels = 1 + round(log2(finalTime / hL));
	}

	for (int i = nLevels - 1; i > -1; i--){	
		nSteps.push_back(pow(2, i));		
		timesteps.push_back(finalTime / nSteps.back());
		nStages.push_back(std::ceil(std::max(2.0, sqrt(3.0 * finalTime * rho) * pow(2.0, -static_cast<double>(i) / 2))));
	}

	int count = 0;
	for (auto it : timesteps) {	
		nTrajectories.push_back(ceil(pow(it, 2 * order) * static_cast<double>(nLevels) / pow(timesteps.front(), 2 * order))); 		
