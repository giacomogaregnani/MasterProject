...
void ModErr::computePF(unsigned int nParam, unsigned int nMC)
{
    ...
    for (unsigned int i = 0; i < nParam; i++) {
        ...
        plt::named_plot("coarse", timeVec, solCoarse[0]);
        plt::named_plot("fine", timeVec, solFine[0]);
        plt::plot(timeVec, errors[i]);

        auto xSample = parFil.sampleX();
        plt::plot(timeVec, xSample);
        plt::plot(timeVec, obs);
        plt::show();

        for (unsigned int j = 0; j < nMC; j++) {
            plt::plot(timeVec, solFine[j]);
        }
        plt::show();
