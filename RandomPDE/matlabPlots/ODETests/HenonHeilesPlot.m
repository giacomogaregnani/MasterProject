...
WTraj = 4.2; WHor = 14; H = 4; offset = 0.05;
...
figStepTraj = createFigure(WTraj, H, 'enhanced',enhanced);
...
set(figStepTraj,'paperpositionmode','auto')
print '-depsc2' '../../../Reports/PaperRTSRK_18/VERSION11/HHStep.eps'


%% Trajectory add

figAddTraj = createFigure(WTraj, H, 'enhanced',enhanced);
...
set(figAddTraj,'paperpositionmode','auto')
print '-depsc2' '../../../Reports/PaperRTSRK_18/VERSION11/HHAdd.eps'
...
figStepChaos = createFigure(WHor, H, 'enhanced',enhanced);
hold on
for i = 1 : nMC
...
set(figStepChaos, 'PaperPositionMode', 'auto')
print '-depsc2' '../../../Reports/PaperRTSRK_18/VERSION11/HHStepChaos.eps'


%% Plot the Hamiltonian for each trajectory

figHam = createFigure(WHor, H, 'enhanced',enhanced);
...
for i = 1 : nMC
    ...
    f(i) = semilogy(time(2:10:end), hamError(1:10:end), 'color', lightgrey);
    hold on     
end
...
for i = 1 : nMC
    ...
    f(nMC + i) = semilogy(time(2:10:end), hamError(1:10:end), 'color', 'k');
end

xlim([0, 600])
xlabel('$t$', 'interpreter', 'LaTex')
ylabel('$|H(p,q) - H(p_0, q_0)|$', 'interpreter', 'LaTex')
legend([f(1), f(nMC + 1)], {'Additive noise', 'Random time step'}, 'Location', 'E', 'orientation', 'vertical', 'interpreter', 'latex')
set(gca, 'fontsize', fontsizeTICK);
set(get(gca, 'xlabel'), 'fontsize', fontsizeLAB);
set(get(gca, 'ylabel'), 'fontsize', fontsizeLAB);
box on

% set(figHam, 'PaperPositionMode', 'auto')
print '-depsc2' '../../../Reports/PaperRTSRK_18/VERSION11/HHStepHam.eps'
