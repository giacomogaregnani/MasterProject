...
for i = 1 : length(hVec)
    ...
    for jj = 1 : M
        ...
        errH1SumEx(i) = errH1SumEx(i) + 1/M * sqrt(integral(@(xx) (graduEx(xx) - gradFunction(xTot, gradUSum, xx)).^2, 0, 1));
...
% fig = createFigure(W, H, 'enhanced',enhanced);
% semilogx(hVec, errL2Sum ./ errL2 + errL2Sum2 ./ errL2, 'k<-', 'markersize', 4);
% xlabel('$h$', 'interpreter', 'latex')
...
hold on
loglog(hVec, errH1SumEx, 'kd-', 'markersize', 4);
loglog(hVec, errH1, 'k<-', 'markersize', 4);
loglog(hVec, errH1Sum, 'ko-', 'markersize', 4);
loglog(hVec, 0.5 * hVec, 'k--')
legend({'$|u_h - u|_{H^1}$', '$E |u_h^+ - u|_{H^1}$', '$E|u_h - \tilde u_h|_{H^1}$', '$E |u_h^+ - u_h|_{H^1}$', 'slope 1'}, 'location', 'se', 'interpreter', 'latex', 'fontsize', fontsizeTICK)
xlabel('$h$', 'interpreter', 'latex')
ylabel('$H^1$ error', 'interpreter', 'latex')

% A posteriori constants
fig = createFigure(W, H, 'enhanced',enhanced);
semilogx(hVec, errH1SumEx ./ errH1Ex, 'k<-', 'markersize', 4);
hold on
semilogx(hVec, errH1Sum ./ errH1, 'ks-', 'markersize', 4);
xlabel('$h$', 'interpreter', 'latex')
