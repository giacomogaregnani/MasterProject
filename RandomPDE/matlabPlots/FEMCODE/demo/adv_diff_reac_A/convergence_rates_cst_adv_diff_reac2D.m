...
%
%   -div(a grad(u)) + b * grad(u) + c u(x) = f(x)
%
% on the domain Omega = [0,1]^2 with constant data a, b, c and exact
% solution u(x)
...
% 4) reaction c given by scalar c1
...
for i=1:Nmax
    ...
    [sol{i,j}, femspace{i,j}, mesh{i,j}, vps{i,j}] = elliptic(thismesh, vp);
    dof(i,j) = femspace{i,j}.ndof;
    [h1(i,j), l2(i,j), h1s(i,j)] = ...
        get_H1error_exact(mesh{i,j}, femspace{i,j}, sol{i,j}, @ex_adv_diff_reac1);
end
end
