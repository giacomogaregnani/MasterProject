function [betaSCM, stats] = natural_SCM(RB, scmSample, train)
%NATURAL_SCM SUCCESSIVE CONSTRAINT METHOD IN NATURAL NORM
%
...
if ~exist(RB.scm.file,'file')
  ...
  if train, load(RB.scm.file, 'stats'); end
  k = 1:numel(beta);
end
...
betaSCM = max(bsxfun(@times, betaLB, beta), [], 2);
  ...
  function updateUBLB(zvec, beta, gamma, beta2, LPA, whk)
    for n=whk
      ...
      % message
      ntosolve = sum((betaUB(:,n) >= train * theta) & tryind);
