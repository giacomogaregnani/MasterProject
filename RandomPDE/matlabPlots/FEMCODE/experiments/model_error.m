for it = 0:5
  Ndiv=2^it;
  saveFile = ['~/repos/experiments/comp/e/peps/eps' num2str(Ndiv) '.mat'];
  saveFileRes = ['~/repos/experiments/comp/e/peps/eps' num2str(Ndiv) 'res.mat'];
  ...
  %% adaptive cycle (we reach 2x DOF)
  DOFlimit = 2*size(mesh.elem,1);
  fluidPart = false(size(mesh.elem,1),1);
  fluidPart(1:size(pormesh.elem,1)) = true;
  while (size(mesh.elem,1) < DOFlimit)
  ...
  vp = struct('a', 1, 'f', [0,-1], 'elemtype', 'p2', 'pelemtype','p1',...
    'solver','iuzawapcg','iuzawapcgopt',struct('verbose',2,'adaptive',true));
  [usol, ufemspace, psol, pfemspace, pormesh, vp] = stokes(pormesh, vp);
  
  
  
  
  
  vphom = struct('a',@ahom_e,'f',[0,0,-1],'elemtype','p1','solver','agmg');
  mesh.bdflag = 'neumann';
