...
RB = struct('expName', expName, 'elemtype', umicElem, ...
  'pelemtype', pmicElem, 'getFineAlg', getFineAlg);
...
RB.offline.folder = [RB.expFolder 'offline/'];
RB.offline.save   = false;
RB.offline.refdom = false;
...
RB.offline = catstruct(RB.offline, struct('epstol', 1e-6, 'fakescm', true)); % fake SCM set to true!!!!

if ~exist(RB.affine.file, 'file'),
  decompose(RB);
end
%if ~exist(RB.scm.file, 'file')
%  sampleSettings = struct('Npoints', 10^4, 'regular', false);
...
if ~exist(RB.online.file, 'file')  
  ...
  statsRBoffline = rbStokesOffline_files(RB, rbSample);
end
