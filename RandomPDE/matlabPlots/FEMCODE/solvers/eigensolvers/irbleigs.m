...
M = []; blsz = 3; cholM = 0; dispr = 0; eigvec = []; endpt = 'MON'; K = 3; 
maxdpol = 200; maxit = 100;
nbls = 3; nval = []; zertyp = 'ML'; sigma = 'LE'; sizint = 1; tol = 1d-6; %V = []; 
permM = []; funpar = [];
...
if (nargin > 2) || (isnumeric(A) && nargin > 1)
    Mpos = 2+(~isnumeric(A));
    if ~isstruct(varargin{Mpos})
        M = varargin{Mpos};
        if ~isnumeric(M)
...
if nargin > 1 + ~isnumeric(A) + ~isempty(M)
   options = varargin{2 + ~isnumeric(A) + ~isempty(M):nargin};
   names = fieldnames(options);
   I = find(strcmpi('BLSZ',names));
   if ~isempty(I), blsz = options.(names{I}); end
   I = find(strcmpi('CHOLM',names));
   if ~isempty(I), cholM = options.(names{I}); end
   I = find(strcmpi('DISPR',names));
   if ~isempty(I), dispr = options.(names{I}); end
   I = find(strcmpi('EIGVEC',names));
   if ~isempty(I), eigvec = options.(names{I}); end
   I = find(strcmpi('ENDPT',names)); IENDPT = I;
   if ~isempty(I), endpt = upper(options.(names{I})); end
   I = find(strcmpi('FUNPAR',names));
   if  ~isempty(I), funpar = options.(names{I}); end
   I = find(strcmpi('K',names));
   if  ~isempty(I), K = options.(names{I}); end
   I = find(strcmpi('MAXDPOL',names)); IMAXDPOL = I;
   if ~isempty(I), maxdpol = options.(names{I}); end
   I = find(strcmpi('MAXIT',names));
   if ~isempty(I), maxit = options.(names{I}); end
   I = find(strcmpi('PERMM',names));
   if ~isempty(I), permM = options.(names{I}); end
   I = find(strcmpi('NBLS',names));
   if ~isempty(I), nbls = options.(names{I}); end
   I = find(strcmpi('ZERTYP',names));
   if ~isempty(I), zertyp = upper(options.(names{I})); end
   I = find(strcmpi('SIGMA',names));
   if  ~isempty(I), sigma = upper(options.(names{I})); end
   I = find(strcmpi('SIZINT',names));
   if ~isempty(I), sizint = options.(names{I}); end
   I = find(strcmpi('TOL',names));
   if ~isempty(I), tol = options.(names{I}); end
   I = find(strcmpi('V0',names));
   if ~isempty(I), V = options.(names{I}); end
end 
...
if (~strcmpi(sigma,'SE') && ~strcmpi(sigma,'LE') && ~strcmpi(sigma,'IE'))
   err = 'SIGMA must be SE, LE or a numeric value.';
   if ~isempty(gcbo),close(waithan);eval(output); end, error(err);   
end
if (~strcmpi(endpt,'FLT') && ~strcmpi(endpt,'MON')), error('Unknown value for ENDPT.'),  end
if (~strcmpi(zertyp,'WL') && ~strcmpi(zertyp,'ML')), error('Unknown value for ZERTYP.'), end
if strcmpi(sigma,'IE')
   zertyp = 'WL'; 
...
while (iter <= maxit)
   ...
   if (nbls > 2 && ~strcmpi(sigma,'IE') && ~ninpeigvec) 
   ...
   if strcmpi(ritzconv,'T'), break, end;
...
if iter >= maxit && nargout == 3
   PRGINF(1) = 1;
   NC = K - length(eigval);
   if strcmpi(sigma,'SE'), JI = 1:NC; end
   if strcmpi(sigma,'LE'), JI = Tsz-NC+1:Tsz; end
   if strcmpi(sigma,'IE'), [~,JI] = sort(abs(ritz-nval)); end
...
if ~isempty(eigval)
   ...
   if strcmpi(sigma,'SE'), [~,I] = sort(eigval);  I = I(1:K); end   
   if strcmpi(sigma,'LE')
      [~,I] = sort(eigval); I = flipud(I); I = I(1:K);
   end   
   if strcmpi(sigma,'IE')
      [~,I] = sort(abs(eigval-nval)); I = I(1:K);
      [~,J] = sort(eigval(I)); I = I(J);
   end
   eigval = eigval(I);
end  
...
function [F,T,V,blsz,mprod,singblk] = blanz(A,K,R,permM,V,blsz,eigvec,funpar,mprod,n,nbls,~,sqrteps,tol) 
...
while (J <= nbls)
   ...
   if ~isempty(R)
       ...
       elseif isnumeric(A)
           ...
           error('not implemented for generalized eigenvalues and func.handles');
       end
       F = (F(permM,:)'/R)';
   else
...
switch sigma
   ...
   case 'SE'
      ...
      if dispr ~= 0 
         ...
         [~,JI] = sort(dispeig); dispeig = dispeig(JI); disperr = disperr(JI);
         dispeig = dispeig(1:K); disperr = disperr(1:K);
         fprintf('Iteration: %3d,  Ritz: %18.15f,  Residual: %18.15f\n', ...
             iter, dispeig(1), disperr(1));
   ...
   case 'IE'
      ...
      if dispr ~= 0 
         ...
         [~,JI] = sort(dispeig); dispeig = dispeig(JI); disperr = disperr(JI);
         dispeig = dispeig(1:K); disperr = disperr(1:K);
         fprintf('Ritz:      Residual:      Iteration: %d\n',iter);
         fprintf('%15.5e  \n',[dispeig';disperr']);
      end
end % Switch sigma.
...
function [fLejapts,lcandpts,leftendpt,leftintendpt,lprd,leftLejapt,lindex,maxdpol,nbls,norlpol,nval,...
         rcandpts,rightendpt,rightintendpt,rprd,rightLejapt,rindex,V,flcount] =... 
         applyshifts(blsz,endpt,F,fLejapts,~,K,lcandpts,leftendpt,leftintendpt,...
                     lprd,leftLejapt,lindex,maxdpol,nbls,norlpol,nval,rcandpts,...
                     rightendpt,rightintendpt,ritz,ritzvec,rprd,rightLejapt,rindex,...
                     zertyp,sigma,singblk,sizint,sqrteps,Tsz,T,V,flcount)
...
switch sigma
   ...
   case 'IE'
      ...
      while ((kn < length(Hritz)) && (kp > 1))
         ...
         if ((kn < (find(Hritz<0, 1, 'last' ) - round(K/2))) && (Hritz(kn) > leftendpt))
            if strcmpi(endpt,'FLT'), leftintendpt = Hritz(kn); end 
            if strcmpi(endpt,'MON')
         ...
         if ((kp >(find(Hritz > 0, 1 ) + round(K/2))) && (Hritz(kp) < rightendpt))
            if strcmpi(endpt,'FLT'), rightintendpt = Hritz(kp); end
            if strcmpi(endpt,'MON')
...
switch zertyp
   ...
   case 'WL' 
      if length(fLejapts) + nbls > maxdpol
         ...
         if strcmpi(sigma,'SE'), norlpol = ritz(1); end
         if strcmpi(sigma,'LE'), norlpol = ritz(length(ritz)); end
         if strcmpi(sigma,'IE'), norlpol = 0; end
      end 
...
% Apply nbls-1 shifts.
for J = nbls:-1:2
   [Q,~] = qr(T-pshifts(nbls-J+1)*eye(Tsz));
   T = Q'*(T*Q);
...
if ((sizlcpts > 1) && (~isempty(leftintendpt)))
   IL = find(fLejapts < leftintendpt);[~,KL] = max(fLejapts(IL));      
   if ~isempty(IL(KL))
...
if ((sizrcpts > 1) && (~isempty(rightintendpt)))
   IR = find(fLejapts > rightintendpt);[~,KR] = min(fLejapts(IR));      
   if ~isempty(IR(KR))
...
function [fLejapts,rcandpts,rindex,rprd] = fastleja(fLejapts,numshfts,rcandpts,rindex,rprd,~)
