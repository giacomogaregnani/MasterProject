function [sol, femspace, mesh, vp] = poisson_mon(mesh, vp, options)
%POISSON solves a nonlinear monotone problem given by mesh and vp
...
[vp, mesh] = init_poisson_mon(mesh, femspace, vp);
...
while increment > options.tolerance && countIt < options.maxIt
    ...
    A_op_solold = assemble_stima_mon(mesh, femspace, vp, solold);

    A = assemble_stima_jac_mon(mesh, femspace, vp, solold);
    NA = size(A,1);
