function cmap = CMRmap_weighted(M,w)
...
w = interp1(1:numel(w), w(:), linspace(1,numel(w),M));
w = cumsum(w);
w =  1 + (NC-1)*(w - w(1)) / (w(end) - w(1));

% Interpolate colormap to colormap size
cmap = zeros(M,3);
for c = 1:3
    cmap(:,c) = interp1((1:NC)',CMRref(:,c),w,'spline');
end
