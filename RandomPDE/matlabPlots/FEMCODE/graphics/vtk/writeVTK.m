function writeVTK(filename,elem,node,p,u)
% vtk export
...
fprintf(FID,s,[(dim+1)*ones(nt,1) elem-1]');
...
if nargin>4
	if nt == size(u,1)
		...
		fprintf(FID,s,[u zeros(nt,3-d)]');
	elseif np == size(u,1)
		s='%f %f %f \n';
		fprintf(FID,'VECTORS u float\n');
		fprintf(FID,s,[u zeros(np,3-d)]');
	end
end
