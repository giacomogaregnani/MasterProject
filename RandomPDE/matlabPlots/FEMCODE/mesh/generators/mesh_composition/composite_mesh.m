function mesh = composite_mesh(gfun, shape, M, options)
...
if ~isfield(options,'lc'), options.lc = 0.15; end
if ~islogical(shape)
	if strcmp(shape,'C')
...
gmsh_options.lc = options.lc;
gmsh_options.periodic = false;

shape = repval(shape,[M,M]);
...
mesh.elem = [];
num = 0;
Ngrids = sum(sum(shape));
for i=1:peri
	for j=1:perj
		if ~shape(i,j), continue; end
		...
		for k=1:3
			for m=1:3
				...
				% this is not a general construct!
        % but it works for now! (if boundaries are too complicated, it fails)
        b1 = locmesh{i,j}.border{k,m};
        b2 = locmesh{i2,j2}.border{subk(k),subk(m)};
        nb = min(numel(b1), numel(b2));
        b1 = b1(end-nb+1:end);
        b2 = b2(end-nb+1:end);
        
				[~,ia] = setdiff(b1, changed(:,1));
				changed = [changed; b1(ia), b2(ia)];
			end
		end
		%% UPDATE
