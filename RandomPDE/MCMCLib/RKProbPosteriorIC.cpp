...
RKProbPosteriorIC::RKProbPosteriorIC(double h,
        ...
        p(p)
{
    ...
    for (int i = 0; i < nMC; i++) {
        ...
        RKSolvers[i] = RungeKuttaRandomH(&generators[i], ODE, tableau, h, p);
    }
}

double RKProbPosteriorIC::computePosterior(VectorXd& theta)
{
