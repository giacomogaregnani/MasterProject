...
\begin{figure}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{First component \corr{$y_1(t)$} of the solution of \eqref{eq:Lorenz} with decreasing Gaussian perturbations on the initial condition from top to bottom \corr{($\sigma = 10^{-1}, 10^{-3}, 10^{-5}$, respectively)}.}
...
\begin{proof} \corr{Since $f$ is sufficiently smooth, the map $t \mapsto \Psi_t(y)$ is of class $C^2(\R^+, \R^d)$ and Lipschitz continuous with constant $L_\Psi$ independent of $y$.} Let us expand the functional $\Phi$ computed on the numerical solution as
	...
	\begin{equation}\label{eq:DistanceProbDet}
		...
		\end{aligned}
	\end{equation}
	Let us remark that due \corr{to the smoothness of the flow} we have
...
\begin{lemma}\label{lem:RecurrenceBound} Suppose that for arbitrary $N$ and $k = 0, \ldots, N$ we have
		...
		\end{equation}
		where $h = T / N$, $A > 0$, $B \geq 0$, $r \geq 1$ and \corr{$e_k \geq 0$}, $k = 0, \ldots, N$. Then
		\begin{equation}
		\corr{e_k} \leq e^{AT}\corr{e_0} + \frac{B}{A}(e^{AT} - 1) h^{r-1}.
		\end{equation}
\end{lemma}

\corr{The proof of \cref{lem:RecurrenceBound} follows from the discrete Grönwall inequality.} We can now state the main result on weak convergence. 
...
\begin{lemma}[Local mean square convergence]\label{thm:StrongOrderLocal} Under \corr{\cref{as:hStrong}, \cref{as:PsiStrong} and \cref{as:hImplicit}} the numerical solution $Y_1$ given by one step of the RTS-RK method \eqref{eq:RTS-RK} satisfies 
...
The proof of \cref{lem:ODERepresentation} follows from \corr{the global Lipschitz continuity of $f$} and the Grönwall inequality. We can now prove the main result on mean square convergence.
...
\begin{proof} Thanks to the classic decomposition of the MSE, we have
	...
	\end{equation}
	The variance of the estimator can be trivially bounded \corr{by} exploiting the Lipschitz continuity of $\Phi$ and the independence of the samples \corr{as}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
\corr{where the notation $M=\OO(h^r)$ for a real number $r$ means that there exist constants $C_1$ and $C_2$ such that $C_1 h^r \leq M \leq C_2 h^r$.}
\begin{remark} Let us remark that in order to have uncertainty quantification for a fixed value $h > 0$ it is necessary to draw a sample with $M > 1$, since otherwise the probability distribution over the numerical solution would be a \corr{Dirac} delta. \cref{thm:MSEMonteCarlo} does not provide an indication of how the value of $M$ should be chosen in order to have a good empirical description of the probability measure induced by the RTS-RK method, but still ensures quantitatively that the Monte Carlo estimators drawn from this distribution have a good quality. 
\end{remark}
...
\begin{lemma}\label{lem:BoundEtaJ} \corr{Suppose that \cref{as:hStrong}, \cref{as:hImplicit} and \cref{as:RTSMoments} hold true, and suppose that $0 < h \leq 1$. Then} the random variables $\eta_j$ satisfy
	\begin{equation}
...
Let us furthermore introduce two lemmas, which will be employed for proving long-time conservation of Hamiltonians. \corr{Let us remark that in \cref{lem:Expansion} the values $n, q, N$ indicate generic positive integers.}   
...
\begin{proof} In the following proof, we denote by $C$ a positive constant independent of $h$ and $n$ which can possibly change value from line to line. Let us first consider the modified Hamiltonian $\tilde Q$ and expand the difference $\tilde Q(Y_n) - \tilde Q(y_0)$ in a telescopic sum as
	...
	\end{aligned}
	\end{equation}
	Hence, \corr{by applying the definition \eqref{eq:ModifiedHamiltonianTrunc} of $\tilde Q$ and \eqref{eq:ModifiedHamiltonianTruncRand} of $\hat Q_j$}, we get
	\begin{equation}
	...
	\end{equation}
	Finally, applying the triangle inequality, since \corr{for all $y \in \R^d$ it holds} $\abs{Q(y) - \tilde Q(y)} \leq Ch^q$ by definition of the modified Hamiltonian $\tilde Q$ \corr{and due to \eqref{eq:HamiltonianTheoremSemiResult}} we get
	\begin{equation}
	\begin{aligned}
...
\begin{figure}[t!]
	...
	\end{tabular}
	\caption{Convergence of the \corr{square root of the} MSE of the Monte Carlo estimator for the random time-stepping explicit trapezoidal (ET) (left figure) and fourth-order Runge--Kutta (RK4) (right figure) \corr{with respect to the time step $h$}. The dashed line corresponds to the order predicted in \cref{thm:MSEMonteCarlo} with $M = 10^3$ for ET and $M = 10^4$ for RK4.}
...
\begin{lemma}\label{lem:WeakSDELocal} Under the assumptions of \cref{lem:WeakLocalOrder} and if \cref{as:RegHamiltonian} holds, then
	...
	\end{equation}
	where $C$ is \corr{a} positive constant independent of $h$ and of $y$, $\tilde Y$ is the solution of \eqref{eq:sde_RTS} and $Y_1$ is the numerical solution given by the RTS-RK method after one step.
\end{lemma}
...
\end{theorem} 
\begin{proof} \corr{The proof follows by replacing $\mathcal{L}$ with $\tilde{\mathcal{L}}$ and \cref{lem:WeakLocalOrder} with \cref{lem:WeakSDELocal} in the proof of \cref{thm:weakOrder}.}
\end{proof}
...
In the following, we denote by $\llbracket a, b \rrbracket$ the interval $\llbracket a, b \rrbracket = [a, b]$ if $a < b$ and $\llbracket a, b \rrbracket = [b, a]$ if $a \geq b$. Let us first consider $r \geq 2$ and the function $\gamma_r(x) = x^r e^{-r\kappa/x}$, whose first derivative is given by
	...
	\end{equation}
	\corr{where we exploited that $e^{-r\kappa/x}$ is a growing function of $x$.} The fundamental theorem of calculus gives
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	Taking expectation on both sides and \corr{since by \eqref{eq:BoundEtaJ} it holds} $\abs{\eta_j}^r \leq C\gamma_r(H_j)$ we obtain
	\begin{equation}
	\E\abs{\eta_j}^r \leq C\big(\gamma_r(h) + r M^{r-2} h^{p+r-3/2} (Mh + \kappa)e^{-r\kappa/(Mh)}\big),
	\end{equation} 
	which proves the desired inequality. This is because \cref{as:RTSMoments} \corr{and \cref{as:hStrong}.\ref{as:hStrong_E} imply} that $M \geq 1$, and because $Mh$ can be bounded by \corr{$M$}. Let us now consider $r = 1$. In this case we have for $t \in \llbracket h, H_j\rrbracket$
	\begin{equation}
...
We first expand the square as
	...
	\end{aligned}
	\end{equation}
	Substituting the expressions \corr{\eqref{eq:ApdProofSndExp} and \eqref{eq:ApdProofThirdExp} in \eqref{eq:ApdProofFirstExp}}, we finally get 
	\begin{equation}
		\Big(\sum_{j=0}^{n-1} \Big(\sum_{k=q}^{N-1} a_{jk} + b_j\Big)\Big)^2 = \sum_{j=0}^{n-1} a_{jq}^2 + 2 \sum_{j=1}^{n-1} \sum_{i=0}^{j-1} a_{jq}a_{iq} + R(a) + S(a, b),
	\end{equation}
...
In the following, all the constants are independent of $h$ and $n$, but can depend on $N$ and $q$. Moreover, since $h < 1$, we often apply $h^r \leq h^s$ for $r \geq s$. We first notice that, under \cref{as:PsiStrong} and \cref{as:RegHamiltonian}, we get for all $j = 0, \ldots, n-1$ and $k = q, \ldots, N-1$
	...
	\end{aligned}
	\end{equation}
	almost surely and where $C_\Delta$ is independent of $h$. \corr{Above, we exploited that $Q_{k+1}$ is Lipschitz continuous for all $k=q, \ldots, N+1$ due to \cref{as:RegHamiltonian}.} Let us now consider $R(\Delta)$. \corr{Due to \eqref{eq:RemainderDeltaBound} and to \cref{as:RTSMoments}}, we have
	...
	\end{aligned}
	\end{equation}
	where $C > 0$ is a positive constant. Now, since \corr{in the definition of $R_2(a)$ in \eqref{eq:RemainderDeltaBound} we have $k \geq q+1$ and $l \geq q$, we have here $k+l \geq 2q+1$}. Therefore, there exists a constant $\tilde C_2$ such that
	\begin{equation}
	...
	\end{aligned}
	\end{equation}
	Computing the two factors singularly, we have \corr{due to \eqref{eq:RemainderDeltaBound} and to \cref{as:RTSMoments}}
	...
	\end{equation}
	For the second term $S_2(\Delta, \eta)$, we have \corr{by \eqref{eq:BoundEtaJ}} that $\abs{\eta_i}\leq CH^i e^{-\kappa/H_i}$ and $\eta_j\leq CH^j e^{-\kappa/H_j}$ almost surely. These two bounds are independent for $i \neq j$ and therefore, applying \cref{lem:BoundEtaJ} with $r = 1$, we have for a constant $\hat C_2 > 0$
	\begin{equation}
	...
	\end{equation}
	Finally, we consider the last term $S_4(\Delta, \eta)$. Since \corr{by \eqref{eq:BoundEtaJ} it holds $\abs{\eta_j} \leq CH_j e^{-\kappa/H_j}$} almost surely, and this bound is independent of $H_i$ for $i \neq j$, applying \eqref{eq:RemainderThirdBound} and \cref{lem:BoundEtaJ} we have
	\begin{equation}
	\begin{aligned}
