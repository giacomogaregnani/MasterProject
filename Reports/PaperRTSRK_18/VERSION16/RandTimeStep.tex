...
\begin{remark} In terms of computational cost simulating the two methods \eqref{eq:RTS-RK} and \eqref{eq:AN-RK} are almost equivalent for an explicit integrator, as they imply the same number of function evaluations as the underlying deterministic solver $\Psi_h$. 
...
\begin{assumption}\label{as:PsiStrong} The Runge--Kutta method defined by the numerical flow $\{\Psi_t\}_{t\geq 0}$ satisfies the following properties.
	\begin{enumerate}
		...
		\item\label{as:PsiStrong_Time} The map $t \mapsto \Psi_t(y)$ is of class $C^2(\R^+, \R^d)$ and Lipschitz continuous with constant $L_\Psi$ independent of $y$, i.e.,
...
\begin{lemma}[Weak local order]\label{thm:WeakLocalOrder} Let \cref{as:hStrong}, \cref{as:hImplicit} and \cref{as:PsiStrong} hold. If $\E|H_0^4| < \infty$, there exists a constant $C > 0$ independent of $h$ and $y$ such that for any function $\Phi\in \mathcal C^l_b(\R^d, \R)$, with $l = \max\{q, 3\}$
	\begin{equation}       
...
\begin{proof} Let us expand the functional $\Phi$ computed on the numerical solution as
	...
	\begin{equation}\label{eq:DistanceProbDet}
		...
		\end{aligned}
	\end{equation}
	Let us remark that due to \cref{as:PsiStrong}.\ref{as:PsiStrong_Order} we have
...
\begin{theorem}[Weak order]\label{thm:weakOrder} Let the assumptions of \cref{thm:WeakLocalOrder} and \cref{as:Stability} hold. Then, there exists a constant $C > 0$ independent of $h$ and of the initial condition such that for all functions $\Phi \in \mathcal C_b^l(\R^d,\R)$, with $l = \max\{q, 3\}$
...
\begin{proof} Let us introduce the following notation
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	We then apply \cref{thm:WeakLocalOrder} to the first term and \cref{as:Stability} to the second and denote $e_k \defeq \sup_{u\in\R^d}\abs{W_k(u) - w_k(u)}$, thus obtaining
	\begin{equation}
...
\end{equation}
where $M$ is the number of number of realisations of the numerical solution and we denote by $\{Y_N^{(i)}\}_{i=1}^M$ a set of i.i.d. realisations of the numerical solution. Hence, we are interested in studying the mean square error of the Monte Carlo estimator, which is defined as
\begin{equation}
...
\begin{lemma}\label{lem:BoundEtaJ} Under \cref{as:hStrong}, \cref{as:hImplicit} and \cref{as:RTSMoments} and if $h \leq \bar h < \infty$ the random variables $\eta_j$ satisfy
	\begin{equation}
...
In the following, we denote by $\llbracket a, b \rrbracket$ the interval $\llbracket a, b \rrbracket = [a, b]$ if $a < b$ and $\llbracket a, b \rrbracket = [b, a]$ if $a \geq b$. Let us first consider $r \geq 2$ and the function $\gamma_r(x) = x^r e^{-r\kappa/x}$, whose first derivative is given by
	...
	\end{aligned}
	\end{equation}
	Taking expectation on both sides and using $\abs{\eta_j}^r \leq C\gamma_r(H_j)$ we obtain
	\begin{equation}
	\E\abs{\eta_j}^r \leq C\big(\gamma_r(h) + r M^{r-2} h^{p+r-3/2} (Mh + \kappa)e^{-r\kappa/(Mh)}\big),
	\end{equation} 
	which proves the desired inequlity. This is because \cref{as:RTSMoments} implies that $M \geq 1$, because $e^{-r\kappa/x}$ is a growing function of $x$, so that $e^{-r\kappa/h}\leq e^{-r\kappa/(Mh)}$ and because $Mh$ can be bounded by $M\bar h$. Let us now consider $r = 1$. In this case we have for $t \in \llbracket h, H_j\rrbracket$
	\begin{equation}
