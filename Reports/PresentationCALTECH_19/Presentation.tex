...
\begin{frameT}
	...
	\vspace{0.2cm}
	{\color{ForestGreen} Take-home message:} There exist computational tools which allows to compute cheaply $p^0$ given the multiscale tensor $A^\epl_u$
	}	
\end{frameT}
...
\begin{frameT}
	...
	\only<3>{{\color{NavyBlue} Idea (\cite{NPS12}):} Replace $\mathcal G^\epl$ with $\mathcal G^0$, forward operator associated to
	\begin{equation}
		\left\{
		\begin{alignedat}{2}
		- \nabla \cdot ( A_u^0 \nabla p^0) &= f, &&\quad \text{ in } \Omega, \\
	...
	where 
	\begin{equation}
		\Phi^\epl(u, y) = \frac12 \norm{\mathcal G^\epl(u) - y}^2_\Gamma.
	\end{equation}
	}
\end{frameT}
...
\frametitle{Multiscale elliptic PDEs -- Inverse problems}
	...
	\begin{tikzpicture}
		...
		\node[] at (8, 0) (homh) {$\mathcal G^0_h$ (hom.+FEM)};
...
\frametitle{Multiscale diffusion processes -- Inference problem}
	...
	{\color{NavyBlue} Theoretical result (\cite{AbG19} -- unpublished):} For $\epl \to 0$
	\begin{equation}	
		d_{\mathrm{Hell}}\big(\mu^\epl(\cdot \mid \mathbf y), \mu^0(\cdot \mid \mathbf y)\big) \to 0.
	\end{equation}
	}
...
\only<2>{
\vspace{0.1cm}
{\color{NavyBlue} Solution:} Employ a particle filter (PF) to obtain an unbiased estimator
\begin{equation}
	\widehat{p^0}(\theta \mid \mathbf y) \approx p^0(\theta \mid \mathbf y),
\end{equation}
...
\end{equation}
with $\epl = 0.1$ (not so small) and $\alpha = 1$, $\sigma = 0.5$. In this case
\begin{equation}
...
\begin{figure}[t]
	\centering
	\includegraphics[width = 0.4\textwidth]{Figures/MultiMulti}
	\includegraphics[width = 0.4\textwidth]{Figures/MultiHomo}
