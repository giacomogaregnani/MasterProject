...
\end{equation}
where $W_t$ is the same Brownian motion as in \eqref{eq:SDE_MS}, such that $X_t^\epl \to X_t$ for $\epl\to 0$ in law as random variables in $\mathcal C^0([0, T]; \R)$. In particular, we have $A = K\alpha$ and $\Sigma = K \sigma$, where the coefficient $K$ is given by the formula
...
\end{equation}
where the matrix $M$ is taken from \eqref{eq:MandH} and the vector $\widetilde{h}$ is taken from \eqref{eq:MandHTilde}. For this estimator we will be able to prove a result of unbiasedness under a condition on the filtering parameter $\delta$. Let us remark that for $\widehat A^\epl_k(T)$ we do not need any condition on this parameter. Nevertheless, this second estimator is the minimizer of a likelihood function unlike the first, and will therefore be used in the Bayesian framework. Let us start with an estimate concerning the distance between $X^\epl$ and $Z^\epl$. 
...
\begin{figure}[t]
	\centering
	\begin{tabular}{ccc}
		\includegraphics[]{/OUSubs_s5} & \includegraphics[]{/OUFilt_s5_b1}  & \includegraphics[]{/OUFilt_s5_b5} \\ % & \includegraphics[]{/OUFilt_s5_b10} \\
		\includegraphics[]{/OUSubs_s7} & \includegraphics[]{/OUFilt_s7_b1}  & \includegraphics[]{/OUFilt_s7_b5} \\ % & \includegraphics[]{/OUFilt_s7_b10} \\
		\includegraphics[]{/OUSubs_s10} & \includegraphics[]{/OUFilt_s10_b1}  & \includegraphics[]{/OUFilt_s10_b5} % & \includegraphics[]{/OUFilt_s10_b10}
	\end{tabular}
...
\begin{figure}[t]
	\centering
	\begin{tabular}{ccc}
		\includegraphics[]{/OUBeta_s5} & \includegraphics[]{/OUBeta_s7}  & \includegraphics[]{/OUBeta_s10} \\
	\end{tabular}
...
\begin{figure}[t]
	\centering
	\begin{tabular}{ccc}
		\includegraphics[]{/Bayes_T100} & \includegraphics[]{/Bayes_T200}  & \includegraphics[]{/Bayes_T400} \\
	\end{tabular}
...
\begin{figure}[t]
	\centering
	\begin{tabular}{ccc}
		\includegraphics[]{/KLNothing} & \includegraphics[]{/KLFilt}  & \includegraphics[]{/KLSubs}
	\end{tabular}
...
\begin{table}[t]
	...
	\begin{tabular}{ccccc}
		\toprule
		Coefficient & Truth & No preprocessing  & Filtering  		  & Subsampling  			 \\ 
					& $A$   & $\widehat A^\epl$ & $\widehat A^\epl_k$ & $\widehat A^\epl_\delta$ \\
		\midrule
		...
		\bottomrule
	\end{tabular}
	\caption{Numerical results for the four-dimensional parameter rounded to the second significant figure.}
	\label{tab:KLStyle}
\end{table}
