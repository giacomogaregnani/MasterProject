...
\begin{theorem}\label{thm:DriftDiscrete} Under assumption \corr{add assumptions}, if there exists $\zeta \in (\beta - 1, \beta)$ such that $\delta = [\epl^{-\zeta}]$ and $\gamma > 2\beta - \zeta$ such that $N = [\epl^{-\gamma}]$, where $[\cdot]$ denotes the integer part of a real number, it holds 
	\begin{equation}
...
\begin{lemma}\label{lem:BoundDiff} Under assumptions \corr{add assumptions}
	\begin{equation}
...
\begin{lemma}\label{lem:DiscFirstBound} Under assumptions \corr{add assumptions}, it holds in law
	\begin{equation}
...
\begin{theorem} Under assumptions \corr{add assumptions}, if $\delta = \epl^\zeta$ for $\zeta \in (0, 1+\beta)$ and if $N = [\epl^{-\gamma}]$ for $\gamma > 0$, then 
	\begin{equation}
