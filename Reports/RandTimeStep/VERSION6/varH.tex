...
\begin{proof} Let us define $e_k \defeq \E\norm{Y_k - y(t_k)}$. Thanks to the triangular inequality we have
	...
	\begin{equation}
		e_k \leq (1 + C_1h) e_{k - 1} + C_2 h^{q + 1} + C_3 \E\abs{H_{k-1} - h}.
	\end{equation}
...
\begin{proof} We can consider the random mapping $\vartheta(y_k, h) = \Theta_k$ such that $H_k = h\vartheta(y_k, h) = h\Theta_k$, where $\{\Theta_k\}_{k \geq 0}$ is a sequence of i.i.d. random variables such that $\Theta_k > 0$ a.s., $\E\Theta_k = 1$ and $\Var\Theta_k = h^{2p - 2}$ for any $k$ so that $H_k$ satisfies Assumption \ref{as:hStrong}. Then the probabilistic method \eqref{eq:ProbMethVarH} can be written as $Y_{k+1} = \Psi_{\Theta_k h}(Y_{k})$. Since the mapping $\vartheta(y, h)$ is independent of $y$, we have
	...
	which is the desired result.
\end{proof}
The long-time behavior of symplectic integrators can be analyzed with techniques of backward error analysis \cite{HLW06}. Let us recall that the values $\{Y_k\}_{k\geq 1}$ still approximate the solution $y(t)$ on an evenly spaced time grid with spacing $h$. The flow $\psi_h$ of the one-step probabilistic numerical method can therefore be expressed as
\begin{equation}
	\psi_h(y_0) = \Psi_{\Theta_0 h}(y_0),
\end{equation}
where $\Psi_h$ is the flow of the deterministic method. Hence, expanding the numerical flow, we have
\begin{equation}\label{eq:NumFlowExp}
	\psi_h(y_0) = y_0 + \Theta_0 h f(y_0) + (\Theta_0 h)^2 d_2(y_0)  + (\Theta_0 h)^3 d_3(y_0) + \ldots, 
\end{equation}
where the functions $\{d_i\}_{i\geq 2}$ are determined by the coefficients of the Runge-Kutta method $\Psi_h$ and by the derivatives of $f$. Since $\Theta_0$ is independent of $f$ and $y_0$, and considering $F(y) = \Theta_0 f(y)$, we can rewrite \eqref{eq:NumFlowExp} as
\begin{equation}
	\psi_h(y_0) = y_0 + h F(y_0) + h^2 D_2(y_0)  + h^3 D_3(y_0) + \ldots, 
\end{equation}
where now the functions $\{D_i\}_{i\geq 2}$ depend on the derivatives of $F$. We now consider the modified equation
\begin{equation}\label{eq:ODEHamModif}
	\tilde y' = F(\tilde y) + hF_2(\tilde y) + h^2F_3(\tilde y) + \ldots,
\end{equation}
such that formally $Y_k = \tilde y(kh)$. Since $\psi_h(y_0)$ is symplectic, it is known that there exist functions $\{E_i\}_{i\geq 2}$ such that $F_i = J^{-1}\nabla E_i$, i.e., \eqref{eq:ODEHamModif} is Hamiltonian. The existence of an Hamiltonian modified equation guarantees that all the standard results of long time conservation of energy and linear growth of the error of the numerical solution hold for the probabilistic method. 
...
\noindent{\color{red} \textbf{Alternative analysis}}

Consider the deterministic integrator $\Psi_h(t)$. There exists a modified equation
\begin{equation}
	\tilde y' = f(\tilde y) + h f_2(\tilde y) + h^2 f_3(\tilde y) + \ldots,
\end{equation}
such that (formally) $y_k = \tilde y(kh)$. Now the probabilistic integrator at $k$-th step has flow $\psi_h(Y_k) = \Psi_{\Theta_k h}(Y_k)$, where $\Theta_k$ is a r.v. such that $H_k = \Theta_k h$ and $H_k$ satisfies Assumption \ref{as:hStrong}. Consider the first step, expanding the numerical solution we have
\begin{equation}
	\psi_h(y_0) = y_0 + h\Theta_0 f(y_0) + (h\Theta_0)^2d_2(y_0) + (h\Theta_0)^3d_3(y_0) + \ldots, 
\end{equation}
where the functions $d_i$ are given by the coefficients of $\Psi_h$ and by the derivatives of $f$. We search a modified equation of the form
\begin{equation}
	\tilde Y' = F_1(\tilde Y) + hF_2(\tilde Y) + h^2F_3(\tilde Y) + \ldots,
\end{equation}
such that formally $\tilde Y(h) = Y_1$. Expanding the solution $\tilde Y(h)$ of this modified equation and matching the terms with equal power of $h$ we have
\begin{equation}
\begin{aligned}
	F_1(y_0) &= \Theta_0 f(y_0),\\
	F_2(y_0) &= \Theta_0^2 d_2(y_0) - \frac{1}{2!}F_1'F_1(y_0)\\
			 &= \Theta_0^2 \big(d_2(y_0) - \frac{1}{2!}f'f(y_0)\big) = \Theta_0^2 f_2(y_0),\\
    F_3(y_0) &= \Theta_0^3 d_3(y_0) - \frac{1}{3!}\big(F_1''(F_1, F_1)(y_0) + F_1'F_1'F_1(y)\big) - \frac{1}{2!}\big(F_1'F_2(y_0) + F_2'F_1(y_0)\big)\\
	         &= \Theta_0^3 \Big(d_3(y_0) - \frac{1}{3!}\big(f''(f, f)(y_0) + f'f'f(y)\big) - \frac{1}{2!}\big(f'f_2(y_0) + f_2'f(y_0)\big)\Big)\\
	         &= \Theta_0^3 f_3(y_0),
\end{aligned}
\end{equation}
where we reported the first three terms for the expansion. Iterating, we find that the modified equation for the probabilistic method is linked to the modified equation of the deterministic method at the first time step with the relation
\begin{equation}
	F_i(y) = \Theta_0^i f_i(y),
\end{equation}
where $f_1(y) = f(y)$. Iterating over the time steps, we have the modified equation
\begin{equation}
	\tilde Y' = \alpha_1(t)f(\tilde Y) + h \alpha_2(t)f_2(\tilde Y) + h^2 \alpha_3(t)f_3(\tilde Y) + \ldots,
\end{equation}
where the time-dependent coefficients $\alpha_i(t)$ are defined as
\begin{equation}
	\alpha_i(t) = \sksum_{k\geq 0} \ind{I_k}(t) \Theta_k^i,
\end{equation}
where $I_k$ is the interval $\big[kh, (k+1)h\big)$. In this way, formally, the solution of the modified equation satisfies $\tilde Y(kh) = Y_k$ for all time steps. 

In the Hamiltonian case, it is possible to prove \cite{HLW06} that the modified equation is Hamiltonian, i.e., there exist functions $\{E_i\}_{i\geq 2}$ such that $f_i = J^{-1}\nabla E_i$. Hence in this case we have
\begin{equation}
	\tilde Y' = \alpha_1(t)J^{-1}\nabla E(\tilde Y) + h\alpha_2(t)J^{-1}\nabla E_2(\tilde Y) + \alpha_3(t)h^2J^{-1}\nabla E_3(\tilde Y) + \ldots.
\end{equation}
If the numerical method is symplectic, it is possible to prove a favorable property of linear growth of the strong error.
\begin{theorem} If the order of the symplectic deterministic method $\Psi_h$ satisfies $q \geq 2$, Assumption \ref{as:hStrong} is satisfied with $p \geq q + 1/2$ and the Hamiltonian system is completely integrable, then
	\begin{equation}
		\E \norm{y(t) - Y_k} \leq Cth^q, 
	\end{equation}
	for $t = kh \leq h^{-q}$ and where $C > 0$ is a constant independent of $t$ and $h$.
\end{theorem}

\noindent {\color{red} \textbf{Alternative analysis - end}}

\noindent {\color{green} \textbf{Alternative analysis}}

An equivalent approach to integrating with random time steps system \eqref{eq:ODEHam} is considering the perturbed equation
\begin{equation}
	Y' = \theta(t)J^{-1}\nabla E(Y),
\end{equation}
where $\theta(t)$ is the differential of a time transformation $t \leftrightarrow \tau$, mapping the random times given by the sum of the drawn random time steps into an equispaced grid. In order to ensure existence and uniqueness of the solution $Y(t)$, we consider $\theta(t)$ to be a continuous function, i.e., a linear interpolation in time of the points $\{H_k / h\}_{k\geq 0}$ for any realization of the time steps. The perturbed equation is Hamiltonian with respect to the time-dependent function $\mathcal{E}(t, y) = \theta(t)E(y)$. Nonetheless, it is possible to simply rewrite it as the sum of the original equation and a perturbation term, i.e.,
\begin{equation}\label{eq:ODEHamPerturb}
	Y' = J^{-1}\nabla E(Y) + \big(1 - \theta(t)\big) J^{-1}\nabla E(Y).
\end{equation}
Provided with this formulation, it is possible to state a result on the long time behavior of the path-wise error of the random time-stepping Runge-Kutta method.
\begin{theorem}\label{thm:LongTimeHam} If the random time-stepping technique is applied to a symplectic Runge-Kutta method with uniform random time steps $\{H_k\} \iid \mathcal{U}(h-Kh^p, h + Kh^p)$, with $K > 0$ and $p > 1$, so that Assumption \ref{as:hStrong} is satisfied, and if the Hamiltonian system is completely integrable and sufficiently smooth, then
	\begin{align}	
		\norm{y(t) - Y_k} &\leq Cth^q \quad \text{a.s.},\\
		\abs{I(y_t) - I(Y_k)} &\leq Ch^{q}, \quad \text{a.s.}
	\end{align}
	where $q$ is the order of the deterministic component given in Assumption \ref{as:PsiStrong}.\ref{as:PsiStrong_Order} and $I(y)$ is any first integral of \eqref{eq:ODEHam}.
\end{theorem}
\begin{remark} The hypotheses for the validity of Theorem \ref{thm:LongTimeHam} are quite restrictive and can be found in \cite{HLW06}, where it is shown that in practice the linear growth of error can be detected even when the equation is not completely integrable.
\end{remark}
\begin{proof} We consider \eqref{eq:ODEHamPerturb} where $\theta(t)$ is a linear interpolation of the values $\{H_k\}_{k\geq 0}$. By construction and from the uniform distribution of the random time steps, we have $\abs{\theta(t)} \leq Kh^{p-1}$ and therefore 
	
\end{proof}

\noindent {\color{green} \textbf{Alternative analysis - end}}
